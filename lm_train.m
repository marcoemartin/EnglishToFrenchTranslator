...
for iFile=1:length(DD)
  ...
  for l=1:length(lines)
    ...
    for idx = 1:numel(words)
        % Uni dictionary
        wrd = char(words(idx));
        if isfield(LM.uni, wrd)
            LM.uni.(wrd) = LM.uni.(wrd) + 1;
        else
           LM.uni.(wrd) = 1;
        end
        
        % Bi dictionary
        if ~isfield(LM.bi, wrd)
            LM.bi.(wrd) = struct();
        end
        if idx > 1
            prev =  char(words(idx-1));
            if isfield(LM.bi, prev) && isfield(LM.bi.(prev), wrd)
                LM.bi.(prev).(wrd) = LM.bi.(prev).(wrd) + 1;
            else
                LM.bi.(prev).(wrd) = 1;
            end
        end
